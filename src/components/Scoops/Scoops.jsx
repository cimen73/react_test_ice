...
const Scoops = () => {
  ...
  useEffect(() => {
    // pull species data
    axios
      .get('http://localhost:3060/species')
      .then((res) => setScoopsData(res.data));
  }, []);

  // How many of the same product are in the basket?
  ...
  return (
    <div className="container text-light">
      <h1 className="mt-4">Ice Cream Types</h1>
      <p>$3 each</p>

      <h2>Ice Cream Types Fee: {scoopBasket.length * 3}$</h2>
      <div className="row gap-5 p-3 justify-content-between">
        {scoopsData.map((scoop) => {
          // Finding how many products are in the basket belonging to the category we rendered.
